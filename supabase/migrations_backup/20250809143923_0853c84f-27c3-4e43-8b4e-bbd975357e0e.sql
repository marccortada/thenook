...
create policy "Authenticated users can read happy hours"
  on public.happy_hours for select
  using (auth.role() = 'authenticated');

create policy "Authenticated users can insert happy hours"
...
drop trigger if exists set_happy_hours_updated_at on public.happy_hours;
