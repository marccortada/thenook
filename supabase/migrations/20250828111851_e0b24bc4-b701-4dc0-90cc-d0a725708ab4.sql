...
-- Primero quitar las referencias de empleados en las reservas
UPDATE public.bookings SET employee_id = NULL WHERE employee_id IS NOT NULL;

-- Ahora eliminar registros de empleados
DELETE FROM public.employees;

-- Eliminar registros de perfiles que sean staff (mantener solo clientes y auth básico)
-- Pero antes asegurar que no haya referencias que puedan causar problemas
...
UPDATE public.lane_blocks SET created_by = NULL WHERE created_by IS NOT NULL;
...
-- Comentario: Las tablas quedan vacías pero funcionales, sin empleados ni staff