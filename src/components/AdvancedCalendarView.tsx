...
import { Dialog, DialogClose, DialogDescription, DialogHeader, DialogOverlay, DialogPortal, DialogTitle } from '@/components/ui/dialog';
...
const CenteredDialogContent: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <DialogPortal>
    <DialogOverlay className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0" />
    <DialogPrimitive.Content className="fixed inset-0 z-[101] flex items-center justify-center p-3 sm:p-6 focus:outline-none">
      {children}
...
const AdvancedCalendarView = () => {
  ...
  const [activePicker, setActivePicker] = useState<"service" | "time" | "payment" | "status" | null>(null);

  useEffect(() => {
    if (editingBooking) {
      setActivePicker(null);
      setEditServiceId(editingBooking.service_id || '');
    }
  }, [editingBooking]);

  useEffect(() => {
    const shouldLockScroll = showEditModal || showBookingModal;
  ...
  }, [showEditModal, showBookingModal]);
            ...
            centerLanes.length === 1 ? "grid grid-cols-[60px_1fr]" :
              ...
              {timeSlots.map((timeSlot, timeIndex) => (
                <div key={timeIndex} className="contents">
                  {/* Time label */}
                  <div className="p-0.5 text-center text-xs border-r border-b bg-muted/30 font-medium h-3 flex items-center justify-center">
                    {timeSlot.hour}
                  </div>
          ...
          <div className="grid grid-cols-[50px_repeat(28,1fr)] gap-0 min-w-[1200px]">
            ...
            <div className="sticky top-0 z-10 bg-background border-b">
              ...
              {timeSlots.map((timeSlot, timeIndex) => (
                <div key={timeIndex} className="contents">
                  {/* Time label */}
                  <div className="p-0.5 text-center text-[10px] border-r border-b bg-muted/30 font-medium h-3 flex items-center justify-center">
                    {timeSlot.hour}
                  </div>
                     ...
                     centerLanes.map((lane) => {
                      ...
                      return (
                         <div
                           ...
                           className="relative h-3 border-r border-b hover:bg-muted/30 cursor-pointer transition-colors"
    ...
    <div className="space-y-4">
      ...
      >
        <CenteredDialogContent>
          <div className="relative w-[95vw] sm:w-[min(95vw,900px)] max-h-[90vh] rounded-lg border bg-background shadow-xl flex flex-col overflow-hidden">
            <DialogClose className="absolute right-4 top-4 rounded-full bg-muted/70 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors p-1">
              <X className="h-4 w-4" />
            </DialogClose>
      ...
      {/* Edit Booking Modal */}
      <Dialog
        open={showEditModal && !!editingBooking}
        onOpenChange={(open) => {
          if (!open) {
            setShowEditModal(false);
            setEditingBooking(null);
      ...
      >
        <CenteredDialogContent>
          {editingBooking && (
            <div
              ref={editDialogRef}
              className="relative w-[95vw] sm:w-[min(95vw,900px)] max-h-[90vh] rounded-lg border bg-background shadow-xl flex flex-col overflow-hidden"
            >
              <DialogClose className="absolute right-4 top-4 rounded-full bg-muted/70 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors p-1">
                <X className="h-4 w-4" />
              </DialogClose>
              ...
              <div className="px-6 pt-6 pb-4 border-b bg-background">
                <DialogHeader className="space-y-1.5 p-0">
                  <DialogTitle className="text-xl font-semibold">Editar Reserva</DialogTitle>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="space-y-2">
                    ...
                    <Button
                      ...
                      onClick={() => setActivePicker("service")}
                    >
                      <span className="truncate">
                        {(() => {
                          const currentServiceId = editServiceId || editingBooking?.service_id || '';
                          return services.find((s) => s.id === currentServiceId)?.name ?? "Seleccionar servicio";
                        })()}
                      </span>
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <span>{format(editTime, 'HH:mm')}</span>
                      <ChevronDown className="h-4 w-4 opacity-50" />
                    </Button>
                  </div>
                  <div className="space-y-2 md:col-span-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="w-full justify-between"
                      onClick={() => setActivePicker("payment")}
                    >
                      <span>
                        {(() => {
                          switch (editPaymentStatus) {
              ...
              {editingBooking && activePicker && (
                ...
                >
                  <div className="w-full max-w-md rounded-lg bg-background shadow-2xl border">
                    <div className="flex items-center justify-between border-b px-4 py-3">
                      <h4 className="text-lg font-semibold">
                        {activePicker === "service" && "Seleccionar servicio"}
                        {activePicker === "time" && "Seleccionar hora"}
                        {activePicker === "payment" && "Estado de pago"}
                        {activePicker === "status" && "Estado de reserva"}
                      </h4>
                      <Button variant="ghost" size="icon" onClick={() => setActivePicker(null)}>
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
                    <div className="max-h-[60vh] overflow-y-auto px-4 py-3 space-y-2">
                      {activePicker === "service" && services
                        .filter((service) => service.center_id === (editingBooking?.center_id || '') || !service.center_id)
                        .map((service) => {
                          const currentServiceId = editServiceId || editingBooking?.service_id || '';
                            ...
                            >
                              <div className="flex flex-col text-left">
                                ...
                                <span className="text-xs text-muted-foreground">
                                  {new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' }).format(service.price_cents / 100)} · {service.duration_minutes} min
                                </span>
                              </div>
                            </Button>
                          );
                      ...
                      {activePicker === "time" && timeOptions5m.map((t) => (
                        <Button
                          key={t}
                          variant={t === format(editTime, 'HH:mm') ? "default" : "outline"}
                          className="w-full justify-start"
                          onClick={() => {
                      ...
                      {activePicker === "payment" && (
                        <>
                          {[
                            ...
                            <Button
                              key={status.value}
                              variant={editPaymentStatus === status.value ? "default" : "outline"}
                              className="w-full justify-start"
                              onClick={() => {
                                setEditPaymentStatus(status.value as typeof editPaymentStatus);
                                setActivePicker(null);
                              }}
                            >
                              {status.label}
                      ...
                      {activePicker === "status" && (
                        <>
                          {[
                            ...
                            <Button
                              key={status.value}
                              variant={editBookingStatus === status.value ? "default" : "outline"}
                              className="w-full justify-start"
                              onClick={() => {
                                setEditBookingStatus(status.value as typeof editBookingStatus);
                                setActivePicker(null);
                              }}
                            >
                              {status.label}
